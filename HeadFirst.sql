...
'Ж', '1980-05-09', 'Писатель', 'Пало-Альто, CA', 'Не замужем', 'Каяк', 'Друзья');

INSERT INTO my_contacts
(first_name, email, profession, location)
VALUES
('Пэт', 'patpost@breakneckpizza.com', 'Почтальон', 'Принстон, NJ');
...
INSERT INTO my_contacts
VALUES
('Фанион', 'Стив', 'steve@onionflavoredrings.com', 'М', '1970-01-04', 
'Панк', 'Гровер\' Милл, NJ', 'Не женат', 'Бунтарство', 'Единомышленники, гитаристы');
...
VALUES
('Большое приключение',         'PG', 'Н', 'Н', 'Н', 'Н', 'Н', 'Н', 'Д'),
...
interests = TRIM(RIGHT(interests, (CHAR_LENGTH(interests) - CHAR_LENGTH(interest1) - 1))),
    
interest2 = SUBSTRING_INDEX(interests, ',', 1),

interests = TRIM(RIGHT(interests, (CHAR_LENGTH(interests) - CHAR_LENGTH(interest2) - 1))),

interest3 = SUBSTRING_INDEX(interests, ',', 1),

interests = TRIM(RIGHT(interests, (CHAR_LENGTH(interests) - CHAR_LENGTH(interest3) - 1))),
...
SELECT LOWER(i.interest) FROM 
(SELECT interest1 interest FROM my_contacts
UNION
SELECT interest2 interest FROM my_contacts
UNION
SELECT interest3 interest FROM my_contacts
UNION
SELECT interest4 interest FROM my_contacts) i
GROUP BY i.interest
...
VALUES
(
    'Мэрфи', 'Пэт', '5551239', 'patmurphy@someemail.com', 'X', '1978-04-15', 
    (SELECT prof_id FROM profession WHERE profession = 'учитель'), '10087',
    (SELECT status_id FROM status WHERE status = 'в браке не состоит')
);


CREATE VIEW job_raises AS
SELECT mc.first_name, mc.last_name, mc.email, mc.phone, jc.salary current_salary, 
jd.salary_low desired_salary, desired_salary - current_salary AS raise
FROM job_current jc
